...
class NoticeR extends JPanel implements ActionListener
{
  JPanel top,bot,mid;
  ...
  NoticeR()
  {
    ...
    add(top,"North");
    tit=new JLabel("Notice Registration");
  ...
  void Show()
  {
	try
	{
     ...
     ResultSet rs=st.executeQuery("select max(regno) from notice");
     int pk=0;
     if(rs.next())
      {
		pk=rs.getInt(1);
  ...
  public void actionPerformed(ActionEvent ae)
   ...
   boolean Valid()
   {
	 ...
	 if(r.equals(""))
	 {
	   JOptionPane.showMessageDialog(this,"Enter Stud regi.");
	   flag=false;
	 }
	 else if(r1.equals(""))
	 {
	 ...
	 {
	 	JOptionPane.showMessageDialog(this,"Enter notice");
	 	flag=false;
	 }
	 else if(r3.equals(""))
	 {
	   JOptionPane.showMessageDialog(this,"Enter depatrtment");
	   flag=false;
	 }
	 else if(r4.equals(""))
	 {
   ...
   void Update()
   {
	 try
	 {
	  ...
	  Connection con=DriverManager.getConnection("Jdbc:Odbc:sham");
	  Statement st=con.createStatement();
      String query="Update notice set date1='"+txtd.getText()+"',notice='"+jta.getText()+"',dep='"+txtdep.getText()+"',mob='"+txtmob.getText()+"' where regno="+Integer.parseInt(txtr.getText());
   ...
   void Delete()
   {
	 ...
	 Connection con=DriverManager.getConnection("Jdbc:Odbc:sham");
	 Statement st=con.createStatement();
	 int i=st.executeUpdate("delete from notice where regno="+Integer.parseInt(txtr.getText()));
	 if(i==1)
   ...
   void Find()
   {
	try
	{
	 ...
	 Connection con=DriverManager.getConnection("Jdbc:Odbc:sham");
	 Statement st=con.createStatement();
	 ResultSet rs=st.executeQuery("select * from notice where regno="+Integer.parseInt(txtr.getText()));
	 if(rs.next())
	 {
